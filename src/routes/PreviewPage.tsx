...
import { Alert, Button, Stack, TextField, MenuItem, FormControl, FormLabel, RadioGroup, FormControlLabel, Radio, Checkbox, FormGroup } from '@mui/material'
...
export default function PreviewPage() {
  ...
  React.useEffect(() => {
    ...
    fields.forEach((f) => {
      init[f.key] = f.defaultValue !== undefined ? f.defaultValue : (f.type === 'checkbox' ? [] : '')
    })
    setValues(init)
    setErrors({})
  }, [fields])

  if (!fields.length) return <Alert severity="info">Go to Create and add fields first.</Alert>
  ...
  const validate = (): boolean => {
    ...
    for (const f of fields) {
      ...
      if (rules.includes('required') && (v === '' || v === undefined || v === null || (Array.isArray(v) && v.length === 0))) {
          ...
          if (f.type === 'radio') {
            return (
              <FormControl key={f.id} error={!!err}>
                ...
                <span style={{ color: 'var(--mui-palette-error-main)', fontSize: 12 }}>{err ?? ' '}</span>
              </FormControl>
            )
          }
